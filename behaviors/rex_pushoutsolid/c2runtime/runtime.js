...
(function () {
	...
	behinstProto.pushOutNearest = function (candidates, max_dist) {
		...
		var overlapInst = this.getFirstOverlapInst(candidates);
		if (!overlapInst)
		...
		while (dist <= max_dist) {
			...
			overlapInst = this.getFirstOverlapInst(candidates, overlapInst);
			if (!overlapInst)
				return true;
		}
	...
	behinstProto.getFirstOverlapInst = function (candidates, overlapInst) {
		if (overlapInst && this.runtime.testOverlap(this.inst, overlapInst))
			return overlapInst;
