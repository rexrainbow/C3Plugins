...
(function () {
    ...
    instanceProto.layerxy2canvasxy = function (x, y, layer) {
        pos.x = layer.layerToCanvas(x, y, true) + this.runtime.canvas.offsetLeft;
        pos.y = layer.layerToCanvas(x, y, false) + this.runtime.canvas.offsetTop;
        return pos;
    };

    instanceProto.triggerEvent = function (evetName, info) {
        var event = new CustomEvent(evetName);
        for (var k in info) {
            event[k] = info[k];
        }
